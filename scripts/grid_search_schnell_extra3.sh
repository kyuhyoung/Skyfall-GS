...
# 22 TIFs needed (19 eval + 3 prereq), copies existing prereqs from prev runs
#
...
JOB_FILE="${SCRIPT_DIR}/output/jobs_schnell_extra3.json"

# --------------- Copy existing prereq TIFs from previous runs ---------------
echo -e "${CYAN}Copying existing prereq TIFs from previous runs...${NC}"
COPIED=0

# 1) Copy from extra1/extra2 (new format: ts08_nmin01_nmax02_pass1.tif)
for PREV_DIR in \
    "${SCRIPT_DIR}/output/grid_schnell_extra_20260402_031650" \
    "${SCRIPT_DIR}/output/grid_schnell_extra2_20260402_045017"; do
    if [ -d "$PREV_DIR" ]; then
        for TIF in "$PREV_DIR"/*.tif; do
...
# 2) Copy from first schnell run (old format with tg/sg: ts4_nmin00_nmax03_tg5.0_sg1.00_pass1.tif)
#    schnell ignores tg/sg so pick any one (tg5.0_sg1.00), rename to new format with zero-padded ts
FIRST_SCHNELL="${SCRIPT_DIR}/output/grid_schnell_20260401_154140"
if [ -d "$FIRST_SCHNELL" ]; then
    $PYTHON -c "
import os, glob, shutil
src_dir = '${FIRST_SCHNELL}'
dst_dir = '${OUTPUT_DIR}'
# Find all tg5.0_sg1.00 TIFs (pick one tg/sg combo since all identical)
for f in sorted(glob.glob(os.path.join(src_dir, '*_tg5.0_sg1.00_*.tif'))):
    old_name = os.path.basename(f)
    # ts4_nmin00_nmax03_tg5.0_sg1.00_pass1.tif -> ts04_nmin00_nmax03_pass1.tif
    import re
    m = re.match(r'ts(\d+)_(nmin\d+_nmax\d+)_tg[\d.]+_sg[\d.]+_(pass\d+)\.tif', old_name)
    if m:
        ts_padded = f'ts{int(m.group(1)):02d}'
        new_name = f'{ts_padded}_{m.group(2)}_{m.group(3)}.tif'
        dst = os.path.join(dst_dir, new_name)
        if not os.path.exists(dst):
            shutil.copy2(f, dst)
            print(f'  {old_name} -> {new_name}')
"
    OLD_COPIED=$(ls "${OUTPUT_DIR}"/*.tif 2>/dev/null | wc -l)
    COPIED=$((OLD_COPIED))
fi

echo -e "${GREEN}Copied ${COPIED} existing TIFs as prereqs${NC}"
echo ""
...
echo -e "${CYAN}  22 TIFs needed (19 eval + 3 prereq)${NC}"
